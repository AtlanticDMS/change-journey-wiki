---
title: "Data Insights & Analytics: A Guidebook for SMB Leaders"
author: "Atlantic DMS"
publisher: "AtlanticDMS.com"
type: book
topic: [data-analytics, marketing-intelligence, research]
tags: [type/book, topic/data-analytics, topic/marketing, status/complete, source/book]
year: 2025
series: "Change Journey Vol 3"
isbn: "9798281000246"
date_added:
---

## Summary
A comprehensive, practical guidebook for SMB leaders on marketing intelligence, data analytics, and evidence-based decision-making. Covers the full journey from understanding why data-driven decisions matter, through building an analytics framework and data-driven culture, to advanced techniques like predictive analytics, customer analytics, competitive benchmarking, ROI measurement, data storytelling, and ethical data use. The most technically detailed volume in the series and the operational backbone of Vol 2's data strategy concepts.

## Core argument
Evidence-based decision-making is no longer a luxury for SMBs — it is a survival necessity. Data-driven organisations consistently outperform competitors, and the democratisation of analytics tools means SMBs can now access capabilities once reserved for large enterprises. The key is not collecting more data, but extracting better insights from the right data.

## Relationship to Vol 1 and Vol 2
Vol 3 is the data and analytics layer that sits beneath Vol 2's digital transformation strategy and Vol 1's change management framework.

- Vol 3's [[Data-Driven Culture]] is the analytics expression of Vol 2's [[Digital Culture]] and Vol 1's [[Human Side of Change+]]
- Vol 3's [[Data Quality and Integrity]] is the operational foundation for Vol 2's [[Data Analytics]]
- Vol 3's [[Predictive Analytics]] is the advanced application of Vol 2's [[Machine Learning]]
- Vol 3's [[Customer Analytics]] extends Vol 2's [[Customer Segmentation]]
- Vol 3's [[Marketing ROI Measurement]] gives metrics teeth to Vol 1's [[KPIs for Change]]
- Overcoming resistance to data adoption (Ch. 4.5) directly applies Vol 1's [[ADKAR Model]] and [[Change Coalition]]
- Vol 3's [[Concepts/Analytics Framework]] is the operational setup that Vol 2's [[Digital Readiness Assessment]] identifies as a need

## Key concepts
- [[DYOR - Do Your Own Research]] — foundational principle of evidence-based decision-making
- [[Marketing Intelligence]] — evolution from focus groups to AI-powered analytics
- [[Themes/Data-Driven Decision Making]] — the business case and practice of evidence-based choices
- [[Concepts/Analytics Framework]] — the organisational infrastructure for systematic data use
- [[Data-Driven Culture]] — embedding analytics thinking into organisational DNA
- [[Data Quality and Integrity]] — clean data as the foundation of actionable insight
- [[Customer Analytics]] — segmentation, CLV, churn prediction, and personalisation
- [[Competitive Intelligence]] — benchmarking, market research, and competitive positioning
- [[Predictive Analytics]] — forecasting future outcomes from historical patterns
- [[Data Storytelling]] — translating analytical findings into compelling narratives
- [[Marketing ROI Measurement]] — attribution models, budget optimisation, and performance metrics
- [[Ethical Data Use]] — privacy, compliance, bias awareness, and responsible analytics
- [[Research Methods for SMBs]] — qualitative and quantitative tools scaled for limited resources
- [[Cognitive Biases in Research]] — 10+ biases that distort data collection and interpretation

## Key people referenced
- [[Peter Drucker]] — "What gets measured gets managed"
- [[Seth Godin]] — "Don't find customers for your products, find products for your customers"
- [[W. Edwards Deming]] — "In God we trust, all others must bring data"

## Themes this book contributes to
- [[Data as Organisational Asset]]
- [[Customer-Centricity]]
- [[Continuous Improvement]]
- [[Technology as Business Strategy]]
- [[Human Side of Change]]
- [[Evidence-Based Leadership]]

## Memorable quotes
- [[Quote - Garbage in garbage everywhere]]
- [[Quote - What gets measured gets managed]]
- [[Quote - Data is not just numbers but a story]]

## My takeaways


## Questions this book raises
- How does a solo-operator SMB prioritise between all these analytics capabilities?
- At what data maturity level does predictive analytics become viable?
- How does the ethical data framework interact with competitive intelligence gathering?

## Links to other books
- Extends: [[AI and Digital Transformation - A Guidebook for SMB Leaders]] (Vol 2 — digital strategy)
- Extends: [[Change Management - A Guidebook for SMB Leaders]] (Vol 1 — human change)
- Agrees with: [[The Lean Startup]] (test, learn, iterate)
